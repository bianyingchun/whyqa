  ...
  <w-layout title="问题测试"
    ...
    <div slot="siderbar">
      <div class="question_list">
        <div class="list_item"
             v-for="(item,index) in origin_questions"
             :key="index"
             @click="set_current_origin_question(index)">{{item}}</div>
      </div>
    </div>
    <div slot="content"
         class="block_wraper">
      <div class="origin_question">
        <div class="title">原始问题<span class="hidden_btn"
                @click="toggle_ignore_origin">
            {{ignore_origin ? '显示':'隐藏'}}</span>
        </div>
        <div class="text"
             v-if="!ignore_origin">{{test_record.origin ? test_record.origin.text:''}}</div>
      </div>
      <div class="test_wraper">
        <div class="title">测试问题</div>
        <div class="input_group">
          <input placeholder="请输入测试问题"
                 class="test_input"
                 type="text"
                 v-model="test_record.test" />
          <button class="test_btn"
                  @click="get_result()">确定</button>
        </div>
      </div>
      <div class="result_wraper">
        ...
        <div class="result_content">
          ...
          <div class="similar_qs">
            <p v-for="(item, index) in test_record.sorted_sim_qs"
               :key="index">{{item.text}}</p>
          </div>
        </div>
        <div class="check_radios">
          <label><input name="matched"
                   type="radio"
                   :value="correctness[1]"
                   checked
                   v-model="test_record.correctness" />正确 </label>
          <label><input name="matched"
                   type="radio"
                   :value="correctness[0]"
                   v-model="test_record.correctness" />错误</label>
        </div>
      </div>
      <div class="corrections"
           v-if="!test_record.correctness && test_record.result">
        <div class="title">应匹配为:</div>
        <div class="corrections_list">
          <div :class="['correct_item', test_record.correct && test_record.correct.id === item.id ? 'active':'']"
               @click="set_correct(item)"
               v-for="item in correct_list"
               :key="item.id">{{item.text}}</div>
        </div>
      </div>
      <div>
        <button class="save_btn"
                v-bind:disabled="!save_enable"
                @click="save_record()">保存记录</button>
      </div>
      <div class="test_records">
        ...
        <div class="table_wraper">
          <table border="1"
            ...
            <tbody>
              <tr v-for="(item, index) in record_list"
                  :key="index">
...
export default {
  ...
  watch: {
    'test_record.correctness': function (newval) {//监听correctness变化,每次correctness变成1时correct = null
      if (newval) {
